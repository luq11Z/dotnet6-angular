...
export class CheckoutPaymentComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    this.stripe = Stripe('pk_test_51KrQV6DuKv62FLRBXe4dYZJPRfO0qLJMCw7MToMRWgZBn2KyZnE8ZFFkDBVX7MTVwh2A8jGv0yuavmuHNB8YSLFi007Gwhatij');
  ...
  onChange(event) {
    ...
    switch (event.elementType) {
      ...
      case 'cardExpiry':
        this.cardExpired = event.complete;
        break;
  ...
  private async createOrder(shoppingCart: IShoppingCart) {
    ...
    return lastValueFrom(order$); //as we want the complete result of the promise we use lastValueFrom
  }
